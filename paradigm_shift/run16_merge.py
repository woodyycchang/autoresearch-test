...
Reads : paradigm_shift/run_016/logs/atoms.json   (from AGENT 1)
        paradigm_shift/run_016/run16_rules.json
        paradigm_shift/run_016/direction_params.json  (current epoch)
Writes: paradigm_shift/run_016/logs/candidates.json
        paradigm_shift/run_016/logs/merge_<cand_id>.json   (full envelopes)

Usage: python3 paradigm_shift/run16_merge.py
"""
...
def tempdir() -> str:
    global _TMP
    if _TMP is None:
        _TMP = tempfile.mkdtemp(prefix="run16_opus_")
    return _TMP
...
def main() -> int:
    ...
    out = {"run_id": "run_016", "epoch": EPOCH,

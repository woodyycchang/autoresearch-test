#!/usr/bin/env python3
"""Run 17 AGENT 2 (merger): real Opus subprocess merges -> candidates.json.

Inherits the PROVEN Run 16 merge mechanics and ADDS Run 17's transparency
requirement: every merge must carry a reasoning_trace so the merge decision is
not a black box (R9).

  * R3: subprocess in a throwaway NON-GIT temp dir, all tools disallowed.
  * R4: force ONE raw JSON object (no prose/fences); robust parse, 1 retry.
  * Gate-4 grounding: primary_quote must be a real >=30-char substring of the
    named atom's text; verified here and re-checked by MAIN.
  * R9: the Opus output MUST include a reasoning_trace{step,inputs_seen,reasoning,
    decision,confidence,could_be_wrong_if} explaining the merge.

Reads : paradigm_shift/run_017/logs/atoms.json   (from AGENT 1)
        paradigm_shift/run_017/run17_rules.json
Writes: paradigm_shift/run_017/logs/candidates.json
        paradigm_shift/run_017/logs/merge_<cand_id>.json   (full envelopes)

Usage: python3 paradigm_shift/run17_merge.py
"""
...
PROMPT = """You are AGENT 2 (merger) in a transparent multi-agent niche-finding pipeline.
Merge two research atoms into ONE candidate paradigm-shift niche in AI / LLMs.

ATOM A (id {a_id}): "{a_text}"
ATOM B (id {b_id}): "{b_text}"

Combine the mechanism in ATOM A with the mechanism/principle in ATOM B into a single
concrete, testable research niche. Then, BECAUSE THIS RUN DEMANDS FULL REASONING
TRANSPARENCY, also expose your middle reasoning: what mechanism transfer you attempted,
why, and what (if anything) makes the combination non-trivial rather than a surface
analogy (vocabulary dressing).

Output ONLY a raw JSON object - NO prose, NO markdown, NO code fences. EXACTLY these keys:
{{"mechanism": "1-2 sentences; MUST use concrete causal verbs (induces, produces, causes, transforms, regulates, activates, inhibits, routes)", "transfer": "1 sentence: what transfers from A to B", "open_problem": "1 sentence: the concrete open research question", "niche_name": "<=12 word title", "primary_quote": "a substring of AT LEAST 30 characters copied EXACTLY and VERBATIM from ATOM A's or ATOM B's text above - do not paraphrase, do not invent", "quote_source": "atom_a or atom_b", "reasoning_trace": {{"step": "merge ATOM A x ATOM B", "inputs_seen": "the two atom texts you were given, summarized as the exact mechanisms you read", "reasoning": "the actual reasoning: which mechanism you tried to transfer from A to B, what alternative framings you considered and rejected, and specifically what makes this non-trivial vs a surface/vocabulary analogy", "decision": "the niche you settled on and why it (not an alternative) is the merge", "confidence": "high|medium|low followed by - and a one-clause reason", "could_be_wrong_if": "the concrete condition under which this merge is just a surface analogy or already-trivial"}}}}

The primary_quote MUST appear character-for-character inside the text of the atom named by
quote_source. Output nothing but the JSON object."""
...
def tempdir() -> str:
    global _TMP
    if _TMP is None:
        _TMP = tempfile.mkdtemp(prefix="run17_opus_")
    return _TMP
...
def parse_obj(text: str) -> dict | None:
    """Robust parse: raw, then last fenced block, then first balanced {...}."""
    text = (text or "").strip()
    try:
...
def main() -> int:
    ...
    for idx, (i, j) in enumerate(pairs, 1):
        cid = f"CAND_017_{idx:03d}"
        print(f"[merge] {cid}: {atoms[i]['atom_id']} x {atoms[j]['atom_id']} ...", flush=True)
        m = merge_pair(atoms[i], atoms[j])
        env = m["envelopes"][-1]
        (LOGS / f"merge_{cid}.json").write_text(json.dumps({"cand_id": cid, **m}, indent=2))
        p = m["parsed"] or {}
        candidates.append({
            "cand_id": cid,
        ...
        print(f"  -> session {env.get('session_id')} parse_ok={m['parse_ok']} "
              f"quote_verified={m['quote_verified_substring']} "
              f"trace_complete={m['reasoning_trace_complete']} niche={p.get('niche_name','')!r}")
    out = {"run_id": "run_017", "epoch": EPOCH, "agent": "2_merger",

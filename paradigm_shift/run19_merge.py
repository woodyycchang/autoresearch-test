#!/usr/bin/env python3
"""Run 19 AGENT 3 (merger): pair the sparsest cross-paper sub-mechanism atoms.

Identical proven logic to Run 18 (cross-paper, ranked by combined per-atom hits,
Opus merge with reasoning_trace) with run_019 paths. R3/R4/R9.

Usage: python3 paradigm_shift/run19_merge.py
"""
...
def tempdir():
    global _TMP
    if _TMP is None:
        _TMP = tempfile.mkdtemp(prefix="run19_opus_")
    return _TMP
...
def main():
    ...
    for idx, (combined, mx, aid, bid, i, j) in enumerate(chosen, 1):
        cid = f"CAND_019_{idx:03d}"
        a, b = by_id[aid], by_id[bid]
        print(f"  {cid}: {aid}(h={hits[aid]}) x {bid}(h={hits[bid]}) combined={combined} ...", flush=True)
        m = merge_pair(a, b)
        env = m["envelopes"][-1]
        (LOGS / f"merge_{cid}.json").write_text(json.dumps({"cand_id": cid, "atom_a_id": aid, "atom_b_id": bid, **m}, indent=2))
        p = m["parsed"] or {}
        candidates.append({
    ...
    out = {"run_id": "run_019", "epoch": EPOCH, "agent": "3_merger",
           "generated_at": datetime.now(timezone.utc).isoformat(),
           "chosen_pairs": [{"cand_id": f"CAND_019_{k+1:03d}", "atom_a": c[2], "atom_b": c[3], "combined_atom_hits": c[0]} for k, c in enumerate(chosen)],

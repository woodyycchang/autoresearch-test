...
Reads : run_021/logs/atoms.json, run_021/logs/atom_search.json, run21_rules.json
Writes: run_021/logs/candidates.json + merge_<cid>.json envelopes

Usage: python3 paradigm_shift/run21_merge.py
"""
...
def tempdir():
    global _TMP
    if _TMP is None:
        _TMP = tempfile.mkdtemp(prefix="run21_opus_")
    return _TMP
...
def select_pairs(atoms, hits):
    """Cross-CONCEPT pairs, ranked to PRIORITIZE TIER-B obscurity (the Run-21 breakthrough
    test): more TIER-B atoms in the pair first, then lower combined paper-hits, then atom_id.
    This surfaces the genuinely-unpaired obscure fusions (B x B, then B x A)."""
    pairs = []
    for i in range(len(atoms)):
        for j in range(i + 1, len(atoms)):
            a, b = atoms[i], atoms[j]
            if a["concept_id"] == b["concept_id"]:
                continue  # same concept already co-occurs
            ha, hb = hits[a["atom_id"]], hits[b["atom_id"]]
            n_tierB = (a.get("tier") == "B") + (b.get("tier") == "B")
            # priority: 0 = cross-tier (1 obscure B + 1 ML-mature A = the 'ML-bridge' breakthrough
            # test), 1 = B x B (both obscure), 2 = A x A; then sparsest combined; then deterministic
            prio = 0 if n_tierB == 1 else (1 if n_tierB == 2 else 2)
            pairs.append((prio, ha + hb, max(ha, hb), a["atom_id"], b["atom_id"], i, j))
    pairs.sort(key=lambda p: (p[0], p[1], p[2], p[3], p[4]))
    # normalise to the (combined, max, a_id, b_id, i, j) shape the caller expects
    return [(p[1], p[2], p[3], p[4], p[5], p[6]) for p in pairs]


def merge_pair(a, b):
    prompt = PROMPT.format(a_id=a["atom_id"], a_src=a.get("source_id", a.get("concept_id","")), a_text=a["text"],
                           b_id=b["atom_id"], b_src=b.get("source_id", b.get("concept_id","")), b_text=b["text"])
...
def main():
    ...
    # Run-21: atom_search covers the primary atoms (one per concept) actually searched this
    # epoch; pair only those searched AND mechanism atoms.
    searched = set(hits)
    atoms = [a for a in atoms_all if a["atom_id"] in searched and is_mech.get(a["atom_id"], True)]
    excluded = [a["atom_id"] for a in atoms_all if a["atom_id"] not in searched]
    ...
    out = {"run_id": "run_021", "epoch": EPOCH, "agent": "3_merger",
           "generated_at": datetime.now(timezone.utc).isoformat(),
           "selection_rule": RULES["pair_selection"]["rule"],
           "chosen_pairs": [{"cand_id": f"CAND_021_{k+1:03d}", "atom_a": c[2], "atom_b": c[3],

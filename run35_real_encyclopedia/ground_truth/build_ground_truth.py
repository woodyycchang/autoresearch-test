...
# --- Run 34: SCALE the registry with real web-harvested method families.
# The 12 inline families above are kept verbatim (they anchor the frozen
# fixtures); ~108 real harvested families are appended at import time so the
# whole pipeline (worker, run_cycle) sees the scaled registry. ---
_HARV = os.path.join(os.path.dirname(__file__), "..", "harvest", "harvested_approaches.json")
if os.path.exists(_HARV):
    _existing_ids = {a["id"] for a in KNOWN_APPROACHES}
    for _a in json.load(open(_HARV))["approaches"]:
        if _a["id"] not in _existing_ids:
            _existing_ids.add(_a["id"])
            KNOWN_APPROACHES.append({
                "id": _a["id"], "name": _a["name"],
                "mechanism_tokens": sorted(set(_a["mechanism_tokens"])),
                "problem_tokens": sorted(set(_a["problem_tokens"]))})
...
F = [
    ...
    # ---- Run 33 borderline ADJUDICATION set: comb ~0.667 (borderline, < thr),
    # mech_j ~0.33 (DIFFERENT mechanism) + prob_j ~1.0 (SAME problem), so
    # two_factor does NOT fire and they reach the borderline branch. BN* are
    # genuine niches (a NOVEL mechanism for a known problem); BV* are re-skins of
    # an existing family. Their TOKEN profiles are near-identical -> v5's blanket
    # conservative_reject rejects ALL (right on BV, WRONG on BN); only the
    # semantic reasoning-audit (v6) can separate them. ----
    ("BN1", ["persistent_homology", "homeostatic_regulation"],
     ["feedback", "regulation", "gradient", "search"],
     ["control", "robustness", "stability"], 0.78, 0.2, True, True,
     "borderline_niche", "Persistent-homology (topological shape tracking) as a GENUINELY NEW mechanism for the homeostatic-stability problem: same problem as A6 but a different mechanism -> a real methodological niche, not a feedback-control re-skin."),
    ("BN2", ["topological_data_analysis", "model_generalization"],
     ["ensemble", "noise", "gradient", "search"],
     ["generalization", "overfitting", "robustness"], 0.80, 0.2, True, True,
     "borderline_niche", "Topological-data-analysis as a novel regulariser for the overfitting problem. Different mechanism from ensemble/dropout (A3) -> genuine niche."),
    ("BN3", ["reservoir_computing", "data_integrity"],
     ["redundancy", "encoding", "gradient", "search"],
     ["integrity", "noise", "reliability"], 0.79, 0.2, True, True,
     "borderline_niche", "Reservoir-computing dynamics as a novel mechanism for the data-integrity problem. Different mechanism from ECC redundancy coding (A7) -> genuine niche."),
    ("BV1", ["pid_controller", "thermostat_setpoint_regulation"],
     ["feedback", "setpoint", "gradient", "search"],
     ["control", "robustness", "stability"], 0.76, 0.2, True, False,
     "borderline_reskin", "A PID controller regulating a thermostat around a setpoint IS homeostatic feedback control (A6): a re-skin, NOT a new niche. Token profile mirrors BN1; only semantics reveal it is a variant."),
    ("BV2", ["dropconnect", "overfitting_reduction"],
     ["noise", "randomized", "gradient", "search"],
     ["generalization", "overfitting", "robustness"], 0.77, 0.2, True, False,
     "borderline_reskin", "DropConnect is a direct dropout/ensemble variant (A3) for the same overfitting problem: a re-skin, NOT a new niche. Token profile mirrors BN2."),

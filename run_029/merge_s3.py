#!/usr/bin/env python3
"""Run 29 session 3 - Phase 2 consolidation. Merge phase1_s3 -> banks with
...
tech_added, tech_rej = [], []
for f in sorted(glob.glob(f"{ROOT}/phase1_s3/tech_*.json")):
    ...
    for t in d.get("techniques", []):
        ...
        e = {"name":t.get("name"),"arxiv_id":a,"arxiv_date":t.get("arxiv_date"),"subfield":t.get("subfield"),
             ...
             "verbatim_quote":t.get("verbatim_quote"),"source":t.get("source"),"_source_agent":ag,"_session":3}
...
for f in sorted(glob.glob(f"{ROOT}/phase1_s3/life_*.json")):
    d = json.load(open(f)); ag = d.get("team","?")
    for c in d.get("concepts", []):
        ...
        e={"name":c.get("name"),"domain":c.get("domain"),"open_problem":c.get("open_problem"),
           ...
           "source":c.get("source"),"_source_agent":ag,"_session":3}
...
json.dump({"_schema":"s3 Phase-2 pass: fresh+usable techniques (R13)","count":len(usable),"techniques":usable},
          open(f"{ROOT}/usable_fresh_techniques_s3.json","w"),indent=2,ensure_ascii=False)

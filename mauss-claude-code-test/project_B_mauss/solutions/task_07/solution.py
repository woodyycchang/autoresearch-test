...
Implements `parse(s)` returning the Python value for a JSON string.
Supports objects, arrays, strings (with \\", \\\\, \\n, \\t, \\r, \\/, \\b, \\f, \\uXXXX
escapes), numbers (int/float, including exponents and negatives), true, false,
and null. Raises ValueError on invalid input. Does not use json / ast /
eval / exec.
"""
...
class _Parser:
        ...
        while self.i < self.n:
            ...
            if ch == "\\":
                ...
                if esc == '"':
                    ...
                    hexstr = self.s[self.i : self.i + 4]
                    try:
                        code = int(hexstr, 16)
                    except ValueError:
                        raise ValueError(f"invalid \\u escape: {hexstr!r}")
                    self.i += 4
                    out.append(chr(code))
                else:
                    raise ValueError(f"invalid escape \\{esc}")
            elif ord(ch) < 0x20:
                raise ValueError(f"unescaped control character at position {self.i}")
            else:
                out.append(ch)
                self.i += 1
    ...
    def _parse_number(self):
        ...
        if self.s[self.i] == "0":
            ...
            raise ValueError(f"invalid number at position {self.i}")
        is_float = False
        # fraction
        if self.i < self.n and self.s[self.i] == ".":
            is_float = True
            self.i += 1
            if self.i >= self.n or not self.s[self.i].isdigit():
                raise ValueError("invalid number: trailing '.'")
            while self.i < self.n and self.s[self.i].isdigit():
                self.i += 1
        # exponent
...
def parse(s):
    """Parse a JSON string and return the corresponding Python value."""

...
def niche_check(c: dict, p: dict, approaches: list) -> dict:
    """Decide NICHE_FOUND vs REJECT(variant/saturated/integrity).

    v4 (variant_rule_mode='scalar', borderline_rule='neighbor_set_first'):
      a borderline verdict is resolved by iterating the *set* of neighbour ids
      and acting on the first one -- Python set iteration over strings is
      PYTHONHASHSEED-dependent, so borderline verdicts can flip between
      processes. This is the inherited Run-27 determinism hazard.
    v5 (variant_rule_mode='two_factor', borderline_rule='conservative_reject'):
      a construct is a variant iff it BOTH reuses a known mechanism AND echoes
      that approach's problem (or the combined sim clears the threshold);
      borderline cases are conservatively rejected; everything is computed from
      deterministic maxima -> order-invariant.
    """
    ...
    if mode == "two_factor":
        ...
        if borderline and rule == "conservative_reject":

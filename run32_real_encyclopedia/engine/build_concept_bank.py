...
def build():
    ...
    for domain, items in RAW.items():
        for i, (name, definition, mech, prob) in enumerate(items, start=1):
            cid = f"{domain[:4]}_{i:02d}"
            concepts.append({
                "id": cid,
                "name": name,
                "domain": domain,
                "definition": definition,
                "mechanism_tokens": sorted(set(mech)),
                "problem_tokens": sorted(set(prob)),
            })
    concepts.sort(key=lambda c: c["id"])
    bank = {
        "schema_version": "1.0",
        "lineage": "real-encyclopedia (reconstructed Run 32; see INHERITANCE.md)",
        "n_domains": len(RAW),
    ...
    print(f"wrote {out}: {len(concepts)} concepts across {len(RAW)} domains")
    return bank

...
**Note on naming.** In the actual repo state, `program.md` is the only
program file and already contains the v2-level features (file chain,
mechanical keyword rule, cross-agent verification at step 12). A separate
`program_v2.md` was referenced in the task spec but does not exist; the
v2 features are inlined in `program.md`. Therefore this diff is between
`program.md` (treated as v2) and the new `program_v3.md`.
